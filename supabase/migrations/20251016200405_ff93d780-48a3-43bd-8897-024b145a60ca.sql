...
-- Add task category to checklist templates if not exists
DO $$ BEGIN
  ALTER TABLE checklist_templates ADD COLUMN task_category task_category NULL;
EXCEPTION
  WHEN duplicate_column THEN null;
END $$;

-- Add approval workflow fields to tasks table if not exists
DO $$ BEGIN
  ALTER TABLE tasks ADD COLUMN task_category task_category NULL;
EXCEPTION
  WHEN duplicate_column THEN null;
END $$;

DO $$ BEGIN
  ALTER TABLE tasks ADD COLUMN approval_status text DEFAULT 'draft';
EXCEPTION
  WHEN duplicate_column THEN null;
END $$;

DO $$ BEGIN
  ALTER TABLE tasks ADD COLUMN current_approval_stage integer DEFAULT 0;
EXCEPTION
  WHEN duplicate_column THEN null;
END $$;

DO $$ BEGIN
  ALTER TABLE tasks ADD COLUMN approval_history jsonb DEFAULT '[]'::jsonb;
EXCEPTION
  WHEN duplicate_column THEN null;
END $$;
...
DO $$ BEGIN
  ALTER TABLE tasks ADD COLUMN priority_level task_priority DEFAULT 'medium';
EXCEPTION
  WHEN duplicate_column THEN null;
END $$;

-- Create task notifications table if not exists
