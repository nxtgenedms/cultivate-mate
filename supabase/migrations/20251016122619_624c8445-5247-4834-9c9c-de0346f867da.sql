-- Add template_item_id column to tasks table to link tasks to specific checklist items
ALTER TABLE public.tasks
ADD COLUMN template_item_id uuid REFERENCES public.checklist_template_items(id);
...
-- Create tasks for existing checklist instances that don't have tasks yet
DO $$
DECLARE
  ...
  task_name TEXT;
BEGIN
  -- Loop through checklist instances that don't have tasks
  FOR instance_record IN 
    SELECT DISTINCT ci.id, ci.template_id, ci.batch_id, ci.created_by, ci.week_ending
    FROM checklist_instances ci
    WHERE NOT EXISTS (
      SELECT 1 FROM tasks t WHERE t.checklist_id = ci.id
    )
  LOOP
    -- Get template info
    SELECT ct.template_name, ct.sof_number
